...
const dropdownNavs = [
    {
        label: "Produtos", navs: [
            {
                title: "Assinaturas",
                desc: "conheça nossas assinaturas",
                path: "/",
        ...
        label: "Recurssos", navs: [
            {
                title: "Blog",
                desc: "Visite nosso blog",
                path: "/blog",
                icon: <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
            ...
            // {
            //     title: "Comunidade",
            //     desc: "Entre na nossa comunidade para interagir",
        ...
        label: "Empresa", navs: [
            {
                title: "Sobre Nós",
                desc: "Saiba mais sobre nossa empresa",
                path: "/about",
                icon: <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
    ...
    return (
        <>
                    ...
                    <div className={`nav-menu flex-1 pb-3 mt-8 md:block md:pb-0 md:mt-0 ${state ? 'block' : 'hidden'}`}>
                        <ul className="items-center space-y-6 md:flex md:space-x-6 md:space-y-0 bg-white">
                                        ...
                                        <li key={idx}>
                                            {
                                                item.isDrapdown ? (
                                                    ...
                                                    <Link href={item.path} target={item.title == "blog" ? "_blank" : "_self"} className="block text-gray-700 hover:text-indigo-600">
                                                        {item.title}
                                                    </Link>
                                                )
                                            }
                                            {
                                                item.isDrapdown && drapdownState.idx == idx && drapdownState.isActive ? (
                                                    <div className="mt-6 bg-white inset-x-0 top-20 w-full md:absolute md:border-y md:shadow-md md:mt-0 z-50">
                            ...
                            <div className='flex-1 items-center justify-end gap-x-6 space-y-3 md:flex md:space-y-0'>
                              {!decode ?   <> <li>
                                    <Link href="/login" className="block py-3 text-center text-gray-700 hover:text-amber-800 border rounded-lg md:border-none">
                                        Log in
                                    </Link>
                                </li>
                                <li>
                                    <Link href="/register" className="block py-3 px-4 font-medium text-center text-white bg-amber-900 hover:bg-amber-800 active:bg-amber-700 active:shadow-none rounded-lg shadow md:inline">
                                        Sign in
                                    </Link>
                                </li>
                           </>
            ...
            <div>
                ...
                <Link href="/profile/Assinaturas" className="block text-indigo-600 hover:text-indigo-500 text-xs">Ver Perfil</Link>
            </div>
        </div></>
